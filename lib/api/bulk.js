...
Job.prototype.open = function(callback) {
  ...
  if (!this._jobInfo) {
    ...
    var valid_content_types = ['XML','JSON','CSV','ZIP_XML','ZIP_JSON','ZIP_CSV'];
    var body = [
      ...
      '<jobInfo  xmlns="http://www.force.com/2009/06/asyncapi/dataload">',
        ...
        (this.options.assignmentRuleId ?
          ...
          ''),
        '<contentType>'+(this.options.contentType && valid_content_types.indexOf(this.options.contentType) !== -1 ? this.options.contentType : 'CSV')+'</contentType>',
      '</jobInfo>'
    ].join('');
...
Batch.prototype._setupDataStreams = function() {
  ...
  //console.log("Setting up data streams...", batch.job.options);
  this._uploadStream = new RecordStream.Serializable();
  this._uploadDataStream = this._uploadStream.stream((batch.job.options && batch.job.options.contentType.match(/zip/i) ? 'zip' : 'csv'), converterOptions);
...
Batch.prototype._createRequestStream = function() {
  ...
  //console.log("Creating request stream!");
  ...
  if (!batch.job.options || !batch.job.options.contentType)
    batch.job.options.contentType = 'CSV';

  var content_type = content_types.CSV;
  if (_.keys(content_types).indexOf(batch.job.options.contentType) !== -1) {
      content_type = content_types[batch.job.options.contentType];
  }
  ...
  return bulk._request(request_obj, function(err, res) {
    if (err) {
...
Batch.prototype.execute = function(input, callback) {
  ...
  if (_.isObject(input) && _.isFunction(input.pipe)) { // if input has stream.Readable interface
    ...
    this.upload_data = input;
    this._dataStream.write(input);
